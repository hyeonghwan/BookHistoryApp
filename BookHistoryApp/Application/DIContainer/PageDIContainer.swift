...
class PageDIContainer{
    ...
    func makePageViewController(_ dependency: PageVCViewModel.PageDependencies,
        ...
        let viewModel = PageVCViewModel(dependency, actions)
        viewModel.paragraphValidator?.contentViewModel = viewModel.contentViewModel
        viewModel.paragraphValidator?.pageViewModel = viewModel
        return PageVC.create(with: viewModel)
    }
    
    func makePageDependencies(_ pagingType: PagingType) -> PageVCViewModel.PageDependencies{
        return PageVCViewModel.PageDependencies(validator: makeParagraphVaildator(),
                                                inputViewModel: makeInputViewModel(),
    ...
    //MARK: - TextValidator
    private func makeParagraphVaildator() -> ParagraphValidatorProtocol{
    ...
    private func makeContentViewModel() -> ContentViewModelProtocol{
        return BookContentViewModel(serviece: self.dependency.bookTransferService)
    }
